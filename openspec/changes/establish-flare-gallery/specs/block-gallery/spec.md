...
`/chapters` MUST exist as an Introduction index. It MUST show a short Flare intro (`Preview. Copy.`, six scroll chapters) and MUST NOT name other products. A left rail MUST list Introduction (active on `/chapters`) then the six catalog slugs linking to `/chapters/[slug]`. Below 1024px the rail MUST be off and a compact switcher MUST include Introduction plus the six chapters.
...
- **THEN** they see Introduction with `Preview. Copy.` and six scroll chapters
- **AND** Introduction is the current rail item
- **AND** they do not see other product names
