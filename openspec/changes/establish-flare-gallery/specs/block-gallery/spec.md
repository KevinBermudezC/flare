...
- **THEN** the Preview iframe width is `min(requested, available stage width)` for each control
- **AND** the active control label matches that applied CSS width
...
- **AND** its top aligns with the hovered row, clamped inside the viewport below site nav and the chapter toolbar `.bar`
- **AND** the card MUST NOT overlap `.bar` vertically
- **AND** it MUST NOT open on keyboard focus, reduced motion, coarse pointer, or the current row
